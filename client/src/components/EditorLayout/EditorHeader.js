    ...
    <header className="bg-zinc-900 border-b border-zinc-800 px-5 py-3 flex items-center justify-between">
      <div className="flex items-center space-x-4">
        ...
        <Listbox value={selectedLanguage} onChange={(lang) => onLanguageChange(lang.id)}>
          <div className="relative">
            <Listbox.Button className="flex items-center space-x-2 px-3 py-1.5 rounded-lg 
                                        bg-zinc-800 hover:bg-zinc-700 border border-zinc-700
                                        text-sm text-zinc-300 transition-all duration-200">
              <span>{selectedLanguage.name}</span>
              <ChevronUpDownIcon className="h-4 w-4 text-zinc-500" aria-hidden="true" />
            </Listbox.Button>
            <Transition
              as={Fragment}
              ...
              leaveTo="opacity-0"
            >
              <Listbox.Options className="absolute z-50 mt-2 w-40 rounded-xl bg-zinc-800 border border-zinc-700
                                           py-1 shadow-xl focus:outline-none">
                {languages.map((lang) => (
                  <Listbox.Option
                    key={lang.id}
                    className={({ active }) =>
      ...
      <div className="flex items-center space-x-3">
        ...
        >
          ...
          <span>{showOutput ? 'Hide Output' : 'Output'}</span>
        </button>
        ...
        <button 
          onClick={() => onRunCode()}
          disabled={isLoading}
          className="flex items-center space-x-2 px-4 py-1.5 rounded-lg text-sm font-medium
